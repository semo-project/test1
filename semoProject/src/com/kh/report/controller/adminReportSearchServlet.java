...
public class adminReportSearchServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 총 게시글 수
		listCount = new ReportService().getSearchBoardCount(search);
		...
		ArrayList<Report> list = new ReportService().selectSearchBoardList(pi, search);
		
		if(list != null) {
			request.setAttribute("list", list);
			request.setAttribute("pi", pi);
			request.setAttribute("search", search);
			request.getRequestDispatcher("views/admin/report/adminReport.jsp").forward(request, response);			
		} else {
