...
public class PageInfo {
	...
	private int startPage;			// 현재 페이지 하단에 보여지는 페이징 바의 시작 수
	private int endPage;			// 현재 페이지 하단에 보여지는 페이징 바의 끝 수
	private int maxPage;			// 전체 페이지에서의 가장 마지막 페이지
	private int pageLimit;			// 한 페이지 하단에 보여질 페이지 최대 갯수
	private int workLimit;			// 한 페이지에 보여질 작품최대 갯수
	...
	public String toString() {
		return "PageInfo [listCount=" + listCount + ", currentPage=" + currentPage + ", startPage=" + startPage
				+ ", endPage=" + endPage + ", maxPage=" + maxPage + ", pageLimit=" + pageLimit + ", workLimit="
				+ workLimit + "]";
	}
