...
/**
 * Servlet implementation class SerialListServlet
 */
@WebServlet("/list.se")
public class SerialListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SerialListServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int workLimit;			// 한 페이지에 보여질 게시글 최대 갯수
		
		// * listCount : 총 게시글 갯수
		listCount = new WorkService().getListCount();
		...
		// * workLimit : 한 페이지에 보여질 게시글 최대 갯수
		workLimit = 10;
		 ...
		 */
		maxPage = (int)Math.ceil((double)listCount/workLimit);
		...
		PageInfo pi = new PageInfo(listCount, currentPage, startPage, endPage, maxPage, pageLimit, workLimit);
		...
		ArrayList<Work> list = new WorkService().selectList(pi);
		
		request.setAttribute("list", list);
		request.setAttribute("pi", pi);
		
		RequestDispatcher view = request.getRequestDispatcher("views/work/artSerial.jsp");
